...
app.use((error, req, res, next) => {
  if (error instanceof SyntaxError && "body" in error) {
    return res
      .status(400)
      .json({ status: false, message: "Invalid JSON request body" });
  }
  return next(error);
});
...
const requirePaystackSecret = (res) => {
  ...
  res.status(500).json({
    status: false,
    message:
      "Payment gateway is not configured. PAYSTACK_SECRET_KEY is missing on the backend.",
  });
  return false;
};
...
app.post("/api/paystack/initialize", async (req, res) => {
  try {
    ...
    console.log(
      `Initializing Paystack transaction ${reference} for ${normalizedEmail}`,
    );
...
app.post("/api/paystack/mpesa", (req, res) => {
  res.status(410).json({
    status: false,
    message:
      "This endpoint is deprecated. Use /api/paystack/initialize instead.",
  });
});
